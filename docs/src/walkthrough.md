```@meta
DocTestSetup = :(using ConcreteStructs)
```
...
```jldoctest
julia> @concrete struct Whatever
           a
           b
       end
...
```jldoctest
julia> @concrete struct MetaArray{T,N,A<:AbstractArray{T,N}} <: AbstractArray{T,N}
           array::A
           name
       end

julia> Base.size(x::MetaArray) = size(x.array);

julia> Base.getindex(x::MetaArray, i...) = getindex(x.array[i...]);

julia> abed = MetaArray([8,10,2,5], "Abed")
4-element MetaArray{Int64, 1, Vector{Int64}, String}:
  8
 10
  2
  5
...
```jldoctest banana-stand
julia> @concrete terse mutable struct BananaStand{has_money}
           employees
           manager
       end
```
...
BananaStand{true,Vector{String},String}
```
